<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UcPrintPeceiptCT.ascx.cs" Inherits="WebApplication1.UC5.CashTransfer.UcController.UcPrintPeceiptCT" %>
<div class="col-md-10 col-md-offset-2">
    <h4>Transfer Success!</h4>
    <h4>Do you want print peceipt?</h4>
    <div class="col-md-12" style="padding-top: 80px;">
        <div class="col-md-4 col-lg-offset-8 col" style="padding-top: 20px;">
            <input type="text" class="btn btn-success btn1 col" value="Accept" />
        </div>
        <div class="col-md-4 col-lg-offset-8 col paddingtop1">
            <input type="text" class="btn btn-success btn1 col" value="Reject" />
        </div>
    </div>
</div>
